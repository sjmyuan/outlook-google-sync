...
describe('authHelper', () => {
  ...
  describe('getAuthUrl', () => {
    it('should returu the correct outlook login url', () => {
      ...
      expect(result).to.equal('https://login.microsoftonline.com/common/oauth2/v2.0/authorize?response_type=code&client_id=test%20id&redirect_uri=https%3A%2F%2Fredirect&scope=read%20offline&access_type=offline&prompt=consent&state=test');
    });
    it('should returu the correct google login url', () => {
      const credential = require('./fixtures/google-client.json');
      const oauth2 = oauth.create(credential);
      const result = getAuthUrl(oauth2, 'https://redirect', 'read offline', 'test');
      expect(result).to.equal('https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=test%20id&redirect_uri=https%3A%2F%2Fredirect&scope=read%20offline&access_type=offline&prompt=consent&state=test');
    });
  });
